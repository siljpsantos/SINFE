<?php
	...
	if(isset($_POST['data']) && $_POST['data'] != NULL){
		$venda_all = venda_data($pdo,$_POST['data']); //seleciona vendas por data
		$data = 1;
	}else {
		$venda_all = venda_all($pdo);
	}
	...
	if($data == 0){
		if(isset($_POST['doc']) && $_POST['doc'] != NULL){
			...
			$venda_all = $pdo->query("SELECT * FROM tab_nfe WHERE id_cliente = '".$select[0]['id_cliente']."' ");
	...
	if($data == 0 && $doc == 0){
		...
		if(isset($_POST['mes']) && $_POST['mes'] != NULL){
			$venda_all = venda_mensal($pdo,$_POST['mes']); //seleciona todas as vendas do mes
		}else {
			$venda_all = venda_all($pdo);
		}
	}
  ...
  <?php include "menu_links.php";?>
    ...
    <center>
    	<article id="op" class="grupo" style="width: 1180px">
    		<br />
			<form style="position: relative; float: left" action="venda_lista.php" method="post" >
				<div id="search">
					<span>
			...
			<br /><br /><br />
			<form style="position: relative; float: left" action="venda_lista.php" id="form_mes" method="post">
				<div id="search">
					<span>
			...
			<form style="position: relative; float: left" action="venda_lista.php" method="post">
				<div id="search">
					<span>
						Pesquisa por CPF/CNPJ do Dest.:
					</span>
					<input type="text" name="doc" id="pesquisa_3" placeholder="Pesquisa de vendas"/>
					<img src="assets/images/lupa.png" id="pesquisa_img">
					<input type="submit" style="position: absolute; left: -9999px"/>
				</div>
			</form>
