...
//caso a nota já tenha sido transmitida, passa adiante
...
//caso a nota já tenha sido transmitida e não haja nenhum erro, passa adiante
if (@$xml_q[0]['transmitido_xml'] != 1) {
    ...
    if($mod!='65'){		
        ...
        $std->pesoL = $venda[0]['peso_liq_nfe']==0 ? null : $venda[0]['peso_liq_nfe'];
        $std->pesoB = $venda[0]['peso_bruto_nfe']==0 ? null : $venda[0]['peso_bruto_nfe'];
    ...
    if($mod!='65'){
        ...
        $std = new stdClass();
        $std->nFat = '000035342';
        $std->vOrig = number_format($tot_prod-$tot_desc,2, '.', '');
        $std->vDesc = "";
        $std->vLiq = number_format($tot_prod-$tot_desc+$vTotTrib,2, '.', '');
    ...
    //if($mod=='65'){
        ...
        $std = new stdClass();
        $std->tPag = $venda[0]['tipo_pg_nfe']=="" ? '01' : $venda[0]['tipo_pg_nfe'];
        $std->vPag = $venda[0]['val_pg_nfe']=="" ? number_format($vNF,1,'.','') : number_format($venda[0]['tipo_pg_nfe'],1,'.',''); //Obs: deve ser informado o valor pago pelo cliente
