...
if($ind_prot == 100){
	...
	$pathNFefile = $xml;
	if (! $indSinc) {
	    $pathProtfile = "../nfephp/xmls/NF-e/" . TP_AMB_N ."/temporarias/".date('Ym')."/{$recibo}-retConsReciNFe.xml";
	} else {
	    $pathProtfile = "../nfephp/xmls/NF-e/" . TP_AMB_N ."/temporarias/".date('Ym')."/{$recibo}-retEnviNFe.xml";
	}
	$saveFile = true;
