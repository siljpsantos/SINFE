...
if(@$xml_q[0]['transmitido_xml'] != 1){
    ...
    $tpAmb = TP_AMB; //1=Produção; 2=Homologação
	...
	$cPais = '1058';
	$xPais = 'Brasil';
	$fone = limpa($emitente[0]['telefone_emitente']);
	...
	if($venda[0]['id_cliente'] != 0){
		...
		//if($cliente[0]['nome_razao_social_cliente'] == "sim"){
			if($mod!='55'){
				$indIEDest = '1';
				$IE = limpa($cliente[0]['inscricao_estadual_cliente']);
			}else if($mod!='65'){
				$indIEDest = '9';
				$IE = '';
			}
		...
		$CEP = limpa($cliente[0]['cep_cliente']);
		$cPais = '1058';
		$xPais = 'Brasil';
		$fone = limpa($cliente[0]['telefone_cliente']);
	...
	if($venda[0]['id_retirada'] != 0){
		...
		$CNPJ = limpa($lista_retirada[0]['cnpj_retirada']);
		$CPF = limpa($lista_retirada[0]['cpf_retirada']);
	...
	if($venda[0]['id_entrega'] != 0){
		...
		$CNPJ = limpa($lista_entrega[0]['cnpj_entrega']);
		$CPF = limpa($lista_entrega[0]['cpf_entrega']);
	...
	foreach($lista_produto as $pont=>$row){
		$aP[] = array(
	        ...
	        'vFrete' => '',
	        'vSeg' => '',
	        'vDesc' => ($produtos[$pont]['val_desc_item']!=='0.00' || $produtos[$pont]['val_desc_unit_item']!=='0.00') ? number_format((($row['valor_produto']*$produtos[$pont]['qtd_item'])/100*$produtos[$pont]['val_desc_item'])+($produtos[$pont]['val_desc_unit_item']),2, '.', '') : '',
	        'vOutro' => '',
	        'indTot' => $indi,
	        'xPed' => '16',
	        'nItemPed' => $pont,
            ...
            //echo "<script>alert('$tot_desc');</script>";         
    }
	...
	if($venda[0]['val_transp_nfe']==NULL){
		$vFrete = 0.00;
	}else{
		$vFrete = $venda[0]['val_transp_nfe'];
	}
	$vSeg = '0.00';
	$vDesc = number_format($tot_desc,2,'.','');
	$vII = '0.00';
	$vIPI = '0.00';
	$vPIS = '0';
	$vCOFINS = '0';
	...
	//$vTotTrib = number_format($vICMS+$vST+$vII+$vIPI+$vPIS+$vCOFINS+$vIOF+$vISS+($vNF*0.04), 2, '.', '');
